...
export const useDashboardData = (forceRefresh = false) => {
  ...
  const loadFromCache = useCallback((): DashboardData | null => {
    try {
      ...
      if (cached) {
        ...
        const now = Date.now();
        // Se o cache ainda Ã© vÃ¡lido e nÃ£o forÃ§ou refresh
    ...
    try {
      ...
      if (useCache && !forceRefresh) {
        ...
        if (cached && cached.stats && cached.memberStats) {
          ...
          // Usar requestIdleCallback se disponÃ­vel para nÃ£o bloquear a UI
          const refreshData = () => {
          ...
          if ('requestIdleCallback' in window) {
            ...
            setTimeout(refreshData, 100); // Fallback mais rÃ¡pido
          }
          return;
        }
      }
  ...
  useEffect(() => {
    ...
    if (userChanged && user !== null) {
      ...
      hasLoadedRef.current = false;
      // Limpar cache quando usuÃ¡rio muda
      try {
        sessionStorage.removeItem(CACHE_KEY);
      } catch (e) {
        // Ignorar erro
      }
      console.log('ðŸ‘¤ UsuÃ¡rio mudou, limpando cache e resetando');
    }
  ...
  useEffect(() => {
    return () => {
      ...
      hasLoadedRef.current = false;
      // Limpar cache ao desmontar se forÃ§ado
      if (forceRefresh) {
        try {
          sessionStorage.removeItem(CACHE_KEY);
        } catch (e) {
  ...
  // FunÃ§Ã£o para forÃ§ar refresh (sempre ignora cache)
  const refresh = useCallback(() => {
    hasLoadedRef.current = false; // Resetar para forÃ§ar recarregamento
